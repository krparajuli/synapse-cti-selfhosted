...
# Write to both stdout and stderr so it's visible regardless of how output is captured
...
# Takes the full command output as $1, not via pipe, so callers keep the raw
...
# ─── Step 10: Create admin user in Cortex ────────────────────────────────────
...
# Capture raw output first so we can show it if extraction fails.
# synapse.tools.aha.provision.user prints to stdout:
#   "one-time use URL: ssl://aha:27272/<guid>?certhash=<sha256>"
...
if [[ -z "$ADMIN_ENROLL_URL" ]]; then
  ...
  echo "${_enroll_raw}"
  echo "────────────────────────────────────────"
  die "Failed to generate admin enrollment URL. Check AHA logs: ${COMPOSE} -f ${SCRIPT_DIR}/docker-compose.yml logs aha"
fi
