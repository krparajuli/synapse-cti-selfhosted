...
#   4.  Adds aha.<domain> to /etc/hosts so AHA can resolve its own DNS name
#   5.  Writes initial .env file
#   6.  Starts AHA (Phase 1) and waits for it to be healthy
#   7.  Generates one-time provisioning URLs for Axon, JSONStor, Cortex
#   8.  Writes provisioning URLs into .env
#   9.  Starts Axon + JSONStor (Phase 2) and waits for healthy
#   10. Starts Cortex (Phase 3) and waits for healthy
#   11. Creates the 'admin' user in Cortex with full admin rights
#   12. Generates an AHA enrollment URL for the admin user
#   13. Prints DNS records, connection info, and client enrollment instructions
...
# ─── Step 4: AHA must resolve its own DNS name at startup ────────────────────
log "Checking /etc/hosts for ${AHA_DNS} ..."
if grep -qF "${AHA_DNS}" /etc/hosts; then
  warn "${AHA_DNS} already in /etc/hosts — skipping"
else
  echo "127.0.0.1  ${AHA_DNS}" >> /etc/hosts
  ok "Added 127.0.0.1  ${AHA_DNS} to /etc/hosts"
fi

# ─── Step 5: Write initial .env (provisioning URLs added later) ──────────────
log "Writing .env ..."
...
# ─── Step 12: Generate admin enrollment URL from AHA ─────────────────────────
